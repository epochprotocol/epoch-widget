...
export function EarnIntentWidget(props: EarnIntentWidgetProps) {
  ...
  // One <Modal> renders every view — its content, title, and chrome change with
  // `view`, but the element stays mounted. Rendering a different modal component
  // per view would remount the <dialog>, replaying its open animation (a flicker
  // when swapping views, e.g. picking a token).
  const modalView = (() => {
    if (view === "selectToken") {
      // Miden assets are keyed by faucet id, not a contract address, so they
      // get their own picker rather than the shared EVM one.
      if (fundingSource === "miden" && midenEnabled) {
        return {
          title: "Select Miden asset",
          onBack: backToMain,
          content: (
            <MidenAssetPicker
              assets={
