...
const EARN_TESTNET_CHAIN_IDS = new Set<number>(
  DUMMY_LENDING_DESTINATION_CHAIN_IDS,
);
const EARN_TESTNET_SOURCE_EVM_CHAIN_IDS = new Set<number>(
  DUMMY_LENDING_SOURCE_EVM_CHAIN_IDS,
);
...
export function EarnIntentWidget({
  ...
  const useLivePools =
    networkEnv === "mainnet" && !!resolvedApi.positionsBaseUrl;
  ...
  const withdrawSourceToken =
    ...
    selectedPosition.market.chainId != null
      ...
      : null;
  useEffect(() => {
    if (withdrawSourceChainId !== null)
      setSelectedChainId(withdrawSourceChainId);
  ...
  const ctaState: {
    ...
    if (earnFlow.isQuoting)
      return { action: "disabled", label: "Fetching quote…" };
    if (earnFlow.status === "submitting")
      return {
        action: "disabled",
        label: earnTab === "deposit" ? "Depositing…" : "Withdrawing…",
      };
    if (earnFlow.status === "complete")
      return { action: "disabled", label: "Completed" };
    ...
    if (earnTab === "deposit" && !earnSelectedMarket)
      return { action: "disabled", label: "Select market" };
    if (earnTab === "deposit" && !earnAmount.trim())
      return { action: "disabled", label: "Enter an amount" };
    if (earnTab === "withdraw" && !selectedPosition)
    ...
    if (insufficientMidenBalance && selectedMidenAsset) {
      return {
        action: "disabled",
        label: `Insufficient ${selectedMidenAsset.symbol} balance`,
      };
    }
    if (
      !activeBuildOk ||
    ...
    <div className="flex flex-col gap-2">
      <button
        ...
        className={twcn(
          ...
          ctaToneClasses,
          (!ctaEnabled || isBusy || earnFlow.isQuoting) &&
