...
/**
 * Build an `IntentProps` from flat Trails-style props. Used internally when
 * the caller does not pass a nested `intent={...}` object.
 *
 * The default protocol/action ("transfer"/"pay") is a placeholder — the
 * solver-side action for plain wallet→wallet pay-flows should match what the
 * allocator expects. Callers needing custom protocol routing should pass
 * `intent={...}` directly.
 */
export function buildPayIntentFromFlatProps(input: FlatPayInputs): FlatPayBuild {
  ...
  const intent: IntentProps = {
    ...
    config: {
      protocol: 'transfer',
      action: 'pay',
      extraDataTypestring: 'address recipient',
      extraData: { recipient: toAddress },
      fixedOutput: false,
      ...(isTestnet
