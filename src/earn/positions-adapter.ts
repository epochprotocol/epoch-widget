...
// Concrete shape of the 1delta `/v1/data/lending/user-positions` response.
// Top-level: { success, data: { items: [...], summary: {...} } }.
// Each item is one (lender, chain) bucket and contains nested `data[]` per
// accountId, each holding the full `positions[]` array (one row per supported
// underlying, even when balance is zero).
...
/** Convert a human-readable decimal string into a raw atomic-units string. */
...
/** Find a configs-derived `EpochEarnMarket` to enrich a raw position. */
...
function buildMarket(
  ...
  // Enrich with config-derived market if available (better display name).
  ...
  return {
    ...
    chainId: chainIdStr ? Number(chainIdStr) : configHit?.cfg.chainId ? Number(configHit.cfg.chainId) : undefined,
    lenderKey,
    lenderName,
...
/**
 * Walk the 1delta response and return all positions where the user has a
 * non-zero deposit balance. Zero-balance rows (which the API also returns to
 * advertise borrow capacity per token) are filtered out.
 */
  ...
  for (const item of items) {
    for (const bucket of item.data ?? []) {
      for (const rawPos of bucket.positions ?? []) {
        ...
        if (!market) {
          console.warn('[positions-adapter] dropped row — missing asset metadata', rawPos);
          continue;
        }
...
/** Extract the portfolio rollup card from the response, if present. */
export function oneDeltaPositionsSummary(raw: unknown): EpochEarnPositionsSummary | null {
  const response = asResponse(raw);
...
/** Derive comma-separated chain IDs + lender keys from the loaded configs. */
